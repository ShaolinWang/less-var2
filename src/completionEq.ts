...
import getColor from 'get-css-colors'
import colorRgba from 'color-rgba'
import type { DepValue } from './utils'
...
function provideCompletionItems(
  ...
  for (const i in allColorVars) {
    const value = allColorVars[i]
    let handleValue = value.shift()!.value
    while (value.length) {
      handleValue = handleValue.replace(value[0].key, value[0].value)
      value.shift()
    }
  ...
  for (const i in allInnerColor) {
    const value = allInnerColor[i]
    if (value.slice(0, 4) === 'fade' && !value.includes('@')) {
      ...
      try {
        ...
        console.error(value, handleValue)
      }
  ...
  const label: vscode.CompletionItem[] = []
  for (const i in colorStore) {
